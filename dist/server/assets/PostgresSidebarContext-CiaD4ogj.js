...
import { Ag as usePostgresSqlEditorPersistence, Dg as usePostgresSidebarSchemas, Eg as usePostgresSidebarPanel, Sg as usePostgresSavedQueryScope, Tg as usePostgresSelectedSchema, vg as usePostgresQueryHistory } from "./hooks-BONwG3Mt.js";
import { Rt as MAX_POSTGRES_QUERY_HISTORY_ENTRIES, zt as MAX_POSTGRES_SQL_EDITOR_TAB_TITLE_LENGTH } from "./auth-BPuxYQAc.js";
import { R as useProject } from "./projects-BaTJenfQ.js";
import { d as postgresNav, i as normalizePostgresTableRouteId, p as postgresTableRows, s as parsePostgresTableId } from "./postgres-database-routes-CyTsPbzl.js";
import { L as buildPostgresSelectSql } from "./database-row-inline-edits-CdyGeTxj.js";
import { r as useAuth } from "./RequireAuth-DahioVQe.js";
import { jsx } from "react/jsx-runtime";
import { useNavigate, useParams } from "@tanstack/react-router";
import { createContext, useCallback, useContext, useEffect, useMemo, useRef, useState } from "react";
import { flushSync } from "react-dom";
var DEFAULT_SQL = "SELECT NOW() AS current_time;";
var PostgresSidebarContext = createContext(null);
...
function postgresQuerySelectionKey(kind, id) {
	return `${kind}:${id}`;
}
...
function createTableTab(tableId) {
	...
	return {
		id: crypto.randomUUID(),
		title: table,
		sql: buildPostgresSelectSql(schema, table, 25, 0),
		tableId,
		result: null,
		explainResult: null,
		error: null
...
function PostgresSidebarProvider({ databaseId, children }) {
	const { projectId } = useParams({ strict: false });
	const navigate = useNavigate();
	const { account } = useAuth();
	const { project } = useProject(projectId);
	const { savedQueryLevel, setSavedQueryLevel } = usePostgresSavedQueryScope(databaseId, account, project?.teamId ?? null);
	const { recentQueries, persistRecentQueries } = usePostgresQueryHistory(databaseId, account);
	const { panel, setPanel } = usePostgresSidebarPanel(databaseId, account);
	const { schemas: sidebarSchemas } = usePostgresSidebarSchemas(projectId, databaseId, "");
	const { selectedSchema, setSelectedSchema } = usePostgresSelectedSchema(databaseId, sidebarSchemas, account);
	const { parseInitialEditorState, persistEditorTabState } = usePostgresSqlEditorPersistence(databaseId, account);
	...
	const openTableInSqlEditor = useCallback((tableId) => {
		focusTableRouteTab(normalizePostgresTableRouteId(tableId));
		navigate({
			...postgresNav({
				projectId,
				databaseId
	...
	return /* @__PURE__ */ jsx(PostgresSidebarContext.Provider, {
		value,
		children
	});
}
function usePostgresSidebar() {
	const context = useContext(PostgresSidebarContext);
	if (!context) throw new Error("usePostgresSidebar must be used within PostgresSidebarProvider");
	return context;
}
export { usePostgresSidebar as i, postgresQuerySelectionKey as n, queryPreviewLabel as r, PostgresSidebarProvider as t };
