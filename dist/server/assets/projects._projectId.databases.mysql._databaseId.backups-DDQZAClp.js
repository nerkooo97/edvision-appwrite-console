...
import { If as useMysqlBackupPolicies, Lf as useMysqlBackups, Mf as MYSQL_BACKUPS_PAGE_SIZE } from "./hooks-BONwG3Mt.js";
...
import { t as Route$1 } from "./projects._projectId.databases.mysql._databaseId.backups-DyLAW9Di.js";
...
function getMysqlBackupStatus(status) {
	const statusInfo = {
		pending: {
			label: "Pending",
			icon: Clock,
...
function isManualMysqlBackup(backup) {
...
function View({ projectId, databaseId }) {
	...
	const [backupsPageSize, setBackupsPageSize] = useState(MYSQL_BACKUPS_PAGE_SIZE);
	...
	const { data: policiesData, isLoading: policiesLoading } = useMysqlBackupPolicies(projectId, databaseId);
	const { data: backupsData, isLoading: backupsLoading } = useMysqlBackups(projectId, databaseId, backupsPage - 1, backupsPageSize);
	...
	const createPolicyMutation = useMutation({
		mutationFn: async (items) => {
			...
			return Promise.all(items.map((policy) => projectSdk.mysql.createBackupPolicy({
				databaseId,
	...
	const deletePolicyMutation = useMutation({
		mutationFn: async (policyId) => {
			return sdk.forProject(projectId).mysql.deleteBackupPolicy({
				databaseId,
				policyId
			});
		},
	...
	const createBackupMutation = useMutation({
		mutationFn: async () => {
			return sdk.forProject(projectId).mysql.createBackup({ databaseId });
		},
		onSuccess: () => {
	...
	const deleteBackupMutation = useMutation({
		mutationFn: async (backupId) => {
			return sdk.forProject(projectId).mysql.deleteBackup({
				databaseId,
				backupId
			});
		},
	...
	const bulkDeleteBackupsMutation = useMutation({
		mutationFn: async (backupIds) => {
			const projectSdk = sdk.forProject(projectId);
			await Promise.all(backupIds.map((backupId) => projectSdk.mysql.deleteBackup({
				databaseId,
				backupId
			})));
		},
	...
	const createRestorationMutation = useMutation({
		mutationFn: async (backupId) => {
			return sdk.forProject(projectId).mysql.createRestoration({
				databaseId,
				type: "backup",
				backupId
			});
										...
										children: [
											...
											/* @__PURE__ */ jsx(TableCell, {
												...
												children: policy ? /* @__PURE__ */ jsx("span", {
													...
													children: policy.name || t("Unnamed Policy")
												}) : isManualMysqlBackup(backup) ? /* @__PURE__ */ jsx("span", {
...
export { MysqlBackupsPage as component };
