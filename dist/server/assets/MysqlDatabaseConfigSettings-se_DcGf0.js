...
import { $d as useMysqlActiveConnections, L as useDedicatedDatabaseStorageChart, Zm as isMysqlClientBackend, ch as useUpdateDedicatedDatabaseHa, k as useDedicatedDatabaseCardMetrics, nm as useUpdateMysqlDatabase, q as getDedicatedDatabaseGaugeHeadline, sh as useDedicatedDatabaseReplicas } from "./hooks-BONwG3Mt.js";
...
function MysqlReplicationSyncModePicker({ syncMode, onSyncModeChange, disabled = false }) {
	const t = useT();
...
function useHaEngine(props) {
	return props.haEngine || props.database.engine || (props.replicationSource?.type === "engine" ? props.replicationSource.engine : "mysql");
}
function MysqlDatabaseReplicasCard({ projectId, databaseId, database, canWrite, replicationSource, haEngine }) {
	const t = useT();
	...
	const { connections, isLoading: connectionsLoading, refetch: refetchConnections } = useMysqlActiveConnections(projectId, engine === "mysql" || engine === "mysql" || !engine ? databaseId : null);
	...
	const currentConnections = useMemo(() => connections.filter(isMysqlClientBackend).length, [connections]);
...
function MysqlDatabaseSyncModeCard({ projectId, databaseId, database, canWrite, replicationSource, haEngine }) {
	const t = useT();
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ jsx("div", { className: "border-t border-border" }),
			/* @__PURE__ */ jsx(MysqlReplicationSyncModePicker, {
				syncMode,
...
function MysqlDatabaseNetworkCard({ projectId, databaseId, database, canWrite }) {
	const t = useT();
...
function MysqlDatabasePitrCard({ projectId, databaseId, database, canWrite }) {
	const t = useT();
	const { features } = useConsoleProfile();
...
function MysqlDatabaseStorageCard({ projectId, databaseId, database, canWrite }) {
	const t = useT();
	const updateMutation = useUpdateMysqlDatabase(projectId, databaseId);
	const { data: specificationsData, isLoading: specificationsLoading } = useDatabaseSpecifications(projectId, MYSQL_DATABASE_SPECS_SOURCE);
...
export { MysqlDatabaseSyncModeCard as a, MysqlDatabaseStorageCard as i, MysqlDatabasePitrCard as n, MysqlDatabaseReplicasCard as r, MysqlDatabaseNetworkCard as t };
