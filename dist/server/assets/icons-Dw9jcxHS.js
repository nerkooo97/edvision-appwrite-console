function normalizeForIcon(frameworkKey) {
	if (!frameworkKey || typeof frameworkKey !== "string") return "";
	return frameworkKey.toLowerCase().replace(/\s+/g, "-").replace(/_/g, "-").replace(/\./g, "-").trim();
}
...
function getFrameworkIconFile(frameworkKey) {
	const normalized = normalizeForIcon(frameworkKey);
	return (normalized && FRAMEWORK_ICON_MAP[normalized]) ?? null;
}
export { getFrameworkIconFile as n, FRAMEWORK_ICON_MAP as t };
