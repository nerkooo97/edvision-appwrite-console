import { t as buildInitGlobeConfig } from "./init-globe-theme-DHztu-Dy.js";
import { i as isResolvedThemeDarkChrome, n as getHtmlThemeKey } from "./html-theme-zz5wyKPq.js";
...
var CAMERA_Z = 300;
var ARC_STROKE = .3;
function applyGlobeLand(globe, landColor) {
	globe.hexPolygonsData(globe_default.features).hexPolygonResolution(3).hexPolygonMargin(.35).hexPolygonAltitude(0).hexPolygonUseDots(false).hexPolygonsTransitionDuration(0).hexPolygonColor(() => landColor).polygonsData([]);
}
function markerToRingEntry(marker) {
	return {
		lat: marker.lat,
		lng: marker.lng,
		color: marker.color,
...
function GlobeRenderControl({ active }) {
...
function resolveGlobeProps(globeConfig) {
	return {
...
function Globe({ globeConfig, data, markers = [], active = true }) {
	...
	useEffect(() => {
		...
		globeRef.current.showAtmosphere(resolved.showAtmosphere).atmosphereColor(resolved.atmosphereColor).atmosphereAltitude(resolved.atmosphereAltitude);
	...
	useEffect(() => {
		...
		const { pointSize, arcLength, arcTime, maxRings, rings, polygonColor } = resolvedRef.current;
		const points = [];
		if (countryMarkers.length > 0) for (const [index, marker] of countryMarkers.entries()) points.push({
			size: pointSize,
			order: index + 1,
			color: marker.color,
			lat: marker.lat,
		...
		const filteredPoints = points.filter((point, index, all) => all.findIndex((other) => other.lat === point.lat && other.lng === point.lng && other.color === point.color) === index);
		globeRef.current.arcsData(arcs).arcStartLat((entry) => entry.startLat).arcStartLng((entry) => entry.startLng).arcEndLat((entry) => entry.endLat).arcEndLng((entry) => entry.endLng).arcColor((entry) => entry.color).arcAltitude((entry) => entry.arcAlt).arcStroke(() => ARC_STROKE).arcDashLength(arcLength).arcDashInitialGap((entry) => entry.order).arcDashGap(15).arcDashAnimateTime(() => arcTime);
		globeRef.current.pointsData(filteredPoints).pointColor((entry) => entry.color).pointsMerge(countryMarkers.length === 0).pointAltitude((entry) => entry.altitude).pointRadius((entry) => entry.radius);
		if (countryMarkers.length > 0) {
			const ringMarkers = countryMarkers.filter((marker) => marker.ringMaxRadius > 0);
			globeRef.current.ringsData(ringMarkers.map(markerToRingEntry)).ringColor((entry) => entry.color).ringMaxRadius((entry) => entry.ringMaxRadius).ringPropagationSpeed(RING_PROPAGATION_SPEED).ringRepeatPeriod(arcTime * arcLength / rings);
		} else globeRef.current.ringsData([]).ringColor(() => polygonColor).ringMaxRadius(maxRings).ringPropagationSpeed(RING_PROPAGATION_SPEED).ringRepeatPeriod(arcTime * arcLength / rings);
	}, [
		data,
		isInitialized,
		markers
	...
	useEffect(() => {
		...
		const interval = window.setInterval(() => {
			...
			if (countryMarkers.length > 0) {
				...
				const ringIndexes$1 = ringMarkers.length === 1 ? [0] : genRandomNumbers(0, ringMarkers.length, Math.max(1, Math.floor(ringMarkers.length * 4 / 5)));
				globeRef.current.ringsData(ringIndexes$1.map((index) => markerToRingEntry(ringMarkers[index]))).ringMaxRadius((entry) => entry.ringMaxRadius);
				return;
			}
			const ringIndexes = genRandomNumbers(0, arcs.length, Math.max(1, Math.floor(arcs.length * 4 / 5)));
			globeRef.current.ringsData(arcs.filter((_arc, index) => ringIndexes.includes(index)).map((arc) => ({
				lat: arc.startLat,
				lng: arc.startLng,
				color: arc.color
			})));
...
function GlobeLights({ globeConfig }) {
	if (globeConfig.evenLighting) return /* @__PURE__ */ jsxs(Fragment, { children: [
		...
		/* @__PURE__ */ jsx("directionalLight", {
			...
			intensity: globeConfig.directionalLightIntensity ?? .2
		}),
		/* @__PURE__ */ jsx("directionalLight", {
...
function World({ globeConfig, data, markers, active = true, maxPixelRatio = 1.5, onReady }) {
	return /* @__PURE__ */ jsxs(Canvas, {
		...
		children: [
			/* @__PURE__ */ jsx(WebGLRendererConfig, { maxPixelRatio }),
			/* @__PURE__ */ jsx(CameraSync, {}),
			/* @__PURE__ */ jsx(FirstFrameNotifier, { onReady }),
			/* @__PURE__ */ jsx(GlobeRenderControl, { active }),
			/* @__PURE__ */ jsx(GlobeLights, { globeConfig }, `${globeConfig.evenLighting ? "even" : "dir"}-${globeConfig.ambientLight}-${globeConfig.directionalLeftLight}-${globeConfig.pointLight}`),
			/* @__PURE__ */ jsx(Globe, {
				globeConfig,
				data,
				markers,
...
function useGlobeThemeConfig() {
	...
	const themeKey = useSyncExternalStore(subscribeToTheme, () => mounted ? getHtmlThemeKey() : "pending", () => "pending");
	return {
		config: mounted && themeKey !== "pending" ? buildInitGlobeConfig(isResolvedThemeDarkChrome(themeKey)) : null,
		themeKey
	};
}
export { World as n, useGlobeThemeConfig as t };
