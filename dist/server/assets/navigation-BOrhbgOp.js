...
import { n as isAgentDocsHref, t as isAgentDocsEnabled } from "./agent-docs-feature-COYbd_m1.js";
function isFirewallDocsSlug(slug) {
	return slug === "products/firewall" || slug.startsWith("products/firewall/");
}
function isFirewallDocsPathname(pathname) {
	const normalized = pathname.replace(/\/+$/, "") || "/";
	return normalized === "/docs/products/firewall" || normalized.startsWith("/docs/products/firewall/");
}
function isFirewallDocsHref(href) {
	const path = href.split(/[?#]/, 2)[0] ?? href;
	return path === "/docs/products/firewall" || path.startsWith("/docs/products/firewall/");
}
function isFirewallDocsEnabled() {
	return getActiveProfileFeatures().firewall;
}
const DOCS_LOCAL_SECTION_NAVS = [
	{
...
function isNavGroup$1(item) {
	return "items" in item;
}
function withoutHref$1(navigation, isHiddenHref) {
	return navigation.flatMap((entry) => {
		if (isNavGroup$1(entry)) {
...
function getAllDocsSectionNavs() {
	const byPrefix = /* @__PURE__ */ new Map();
	for (const config of DOCS_SECTION_NAVS) byPrefix.set(config.prefix, config);
	for (const config of DOCS_LOCAL_SECTION_NAVS) byPrefix.set(config.prefix, config);
	let configs = Array.from(byPrefix.values());
	if (!isFirewallDocsEnabled()) configs = configs.filter((config) => config.prefix !== "products/firewall").map((config) => config.prefix === "products/network" ? {
		...config,
		navigation: withoutHref$1(config.navigation, isFirewallDocsHref)
	} : config);
	if (!isAgentDocsEnabled()) configs = configs.filter((config) => config.prefix !== "products/agent").map((config) => config.prefix === "tooling/ai" ? {
		...config,
		navigation: withoutHref$1(config.navigation, isAgentDocsHref)
	} : config);
	return configs.sort((a, b) => a.prefix.localeCompare(b.prefix));
}
const DOCS_GLOBAL_NAV = [
	{ items: [
		{
			label: "Home",
			href: "/docs",
...
const DOCS_PARTNERS_GLOBAL_NAV = [
	{ items: [
		{
			label: "Overview",
			href: "/docs/partners",
...
function getDocsAudienceFromSlug(slug) {
	return slug === "partners" || slug.startsWith("partners/") ? "partners" : "developers";
}
function getDocsAudienceFromPathname(pathname) {
	const normalized = pathname.replace(/\/+$/, "") || "/";
	if (normalized === "/docs/partners" || normalized.startsWith("/docs/partners/")) return "partners";
	return "developers";
}
function isNavGroup(item) {
	return "items" in item;
}
function withoutHref(navigation, isHiddenHref) {
	return navigation.flatMap((entry) => {
		if (isNavGroup(entry)) {
			const items = entry.items.filter((item) => !isHiddenHref(item.href));
			if (items.length === 0) return [];
...
function getDocsGlobalNav(audience) {
	let navigation = audience === "partners" ? DOCS_PARTNERS_GLOBAL_NAV : DOCS_GLOBAL_NAV;
	if (audience === "partners") return navigation;
	if (!isFirewallDocsEnabled()) navigation = withoutHref(navigation, isFirewallDocsHref);
	if (!isAgentDocsEnabled()) navigation = withoutHref(navigation, isAgentDocsHref);
	return navigation;
}
function getDocsAudienceHomeHref(audience) {
	return audience === "partners" ? "/docs/partners" : "/docs";
}
function isDocsNavGroup(item) {
	return "items" in item;
}
function countSectionNavLinks(navigation) {
	let count = 0;
	for (const node of navigation) if (isDocsNavGroup(node)) count += node.items.length;
	else count += 1;
	return count;
}
function getDocsSectionNav(slug) {
	if (!slug) return {
		parent: null,
		navigation: null
	};
	...
	if (best && countSectionNavLinks(best.navigation) > 1) return {
		parent: best.parent,
...
export { getDocsAudienceHomeHref as a, DOCS_SECTION_NAVS as c, isFirewallDocsSlug as d, getDocsAudienceFromSlug as i, isFirewallDocsEnabled as l, isDocsNavGroup as n, getDocsGlobalNav as o, getDocsAudienceFromPathname as r, getAllDocsSectionNavs as s, getDocsSectionNav as t, isFirewallDocsPathname as u };
