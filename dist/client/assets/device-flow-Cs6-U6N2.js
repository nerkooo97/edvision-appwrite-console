...
{% only_dark %}
![The device flow between the device, the OAuth2 server, and a second device like the user's phone or laptop](/images/docs/oauth-server/dark/diagram-device-flow.avif)
{% /only_dark %}
{% only_light %}
...
## Enable device flow for the client {% #enable-client %}
...
# Integrate the device client {% #integrate-device %}

The device client uses HTTP for the device authorization and token requests.

## 1. Start a device authorization {% #start %}
...
# Build the verification page {% #verification-page %}
...
# Finish on the original device {% #finish %}
