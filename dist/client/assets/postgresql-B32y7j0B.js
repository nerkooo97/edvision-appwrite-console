...
Native databases are different from Appwrite databases like [TablesDB](/docs/products/databases/tablesdb), [DocumentsDB](/docs/products/databases/documentsdb), and [VectorsDB](/docs/products/databases/vectorsdb), which are accessed through Appwrite SDKs and platform APIs. A native PostgreSQL database gives you the raw engine. You connect with \`psql\` or any PostgreSQL driver and use the full feature set of PostgreSQL, with no Appwrite abstraction layer in between.

{% info title="Looking for Appwrite data APIs?" %}
If you want Appwrite SDKs, platform permissions, and serverless scaling for app data, start with [Appwrite Databases](/docs/products/databases). Native PostgreSQL is for teams who want a managed PostgreSQL engine they can talk to directly with their own ORM, migrations, and drivers.
{% /info %}
...
| Engine     | Default version | Other supported versions | Default port |
|------------|-----------------|--------------------------|--------------|
| PostgreSQL | 18              | 17                       | 5432         |
...
{% cards %}
{% cards_item href="/docs/products/databases/postgresql/connections" title="Connections" %}
Connect with \`psql\` or any driver. Credentials are rotatable from the Console and API.
{% /cards_item %}
{% cards_item href="/docs/products/databases/postgresql/connection-pooling" title="Connection pooling" %}
Per-database connection pooler with automatic read/write split when high availability is enabled.
{% /cards_item %}
{% cards_item href="/docs/products/databases/postgresql/extensions" title="Extensions" %}
Install PostgreSQL extensions like PostGIS, pgvector, and pg_trgm.
{% /cards_item %}
{% cards_item href="/docs/products/databases/postgresql/backups" title="Backups" %}
Scheduled backups, manual backups, restores, and point-in-time recovery.
{% /cards_item %}
{% cards_item href="/docs/products/databases/postgresql/branches" title="Branches" %}
Spin up an ephemeral copy of your database in seconds from a storage snapshot.
{% /cards_item %}
{% cards_item href="/docs/products/databases/postgresql/high-availability" title="High availability" %}
Up to five synchronous, semi-synchronous, or asynchronous replicas with automatic failover.
{% /cards_item %}
{% cards_item href="/docs/products/databases/postgresql/scaling" title="Scaling" %}
Resize compute online and grow storage automatically as your data grows.
{% /cards_item %}
{% cards_item href="/docs/products/databases/postgresql/network-security" title="Network security" %}
TLS by default, optional IP allowlists, and a dedicated hostname per database.
{% /cards_item %}
{% cards_item href="/docs/products/databases/postgresql/monitoring" title="Monitoring" %}
Live compute, connection, storage, and workload metrics, with an active-connections inspector.
{% /cards_item %}
{% cards_item href="/docs/products/databases/postgresql/maintenance" title="Maintenance" %}
Maintenance windows, online version upgrades, and pause and resume.
{% /cards_item %}
{% /cards %}

# Specifications and pricing {% #specifications %}
...
# Billing and plan requirements {% #plans %}
