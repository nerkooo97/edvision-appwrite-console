...
You can only use Git deployment for Appwrite Sites connected to Git.\xA0[Create a new site with GitHub](/docs/products/sites/quick-start)\xA0or connect your existing site to a GitHub repository in your site’s **Settings**\xA0>\xA0**Git repository**\xA0>\xA0**Connect Git**.
...
When you push a commit to the production branch, usually\xA0\`main\`, a new deployment is created, built, and activated. This means the new deployments **immediately replace the current active deployment**\xA0and can be used on your site’s primary domain.
...
If you need to update your Git configuration, navigate to\xA0**Sites** > your site >\xA0**Settings**\xA0>\xA0**Git repository**.
...
- If you're missing some code files at build time, verify your **build command** and ensure that the build output is included in the Git configuration's\xA0**output directory**. Only files in the output directory folder will be available after deployment.
- If you're self-hosting Appwrite, you will need to configure some\xA0[environment variables](https://appwrite.io/docs/advanced/self-hosting/functions)\xA0to enable Git deployments.
`;export{e as default};