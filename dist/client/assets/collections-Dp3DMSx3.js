...
Because the schema is fixed, VectorsDB has no typed-attribute endpoints like TablesDB columns. You cannot add, update, or delete attributes on a collection. You store your vector under \`embeddings\` and any associated data under \`metadata\`.
...
Head to the **Databases** page, open a [database](/docs/products/databases/vectorsdb/databases), and click **Create collection**. Set a **dimension** that matches the embedding model you plan to use.
...
\`\`\`
\`\`\`bash
appwrite vectors-db get-collection \\
    --database-id "<DATABASE_ID>" \\
    --collection-id "<COLLECTION_ID>"
\`\`\`
{% /multicode %}
...
\`\`\`
\`\`\`bash
appwrite vectors-db update-collection \\
    --database-id "<DATABASE_ID>" \\
    --collection-id "<COLLECTION_ID>" \\
    --name "<NAME>"
\`\`\`
...
\`\`\`
\`\`\`bash
appwrite vectors-db delete-collection \\
    --database-id "<DATABASE_ID>" \\
    --collection-id "<COLLECTION_ID>"
\`\`\`
{% /multicode %}
