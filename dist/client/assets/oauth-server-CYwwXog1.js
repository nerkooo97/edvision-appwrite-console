...
{% only_dark %}
![A third-party app obtaining tokens from your project's OAuth2 server and calling your APIs](/images/docs/oauth-server/dark/diagram-overview.avif)
{% /only_dark %}
{% only_light %}
...
| [RFC 8252](https://datatracker.ietf.org/doc/html/rfc8252) | OAuth for native apps: loopback redirects match on any port, and public clients can register private-use scheme redirect URIs. |
...
{% cards %}
{% cards_item href="/docs/products/auth/oauth-server/quick-start" title="Quick start" %}
Enable the server, register a client, and run your first sign-in end to end.
{% /cards_item %}
{% cards_item href="/docs/products/auth/oauth-server/clients" title="Clients" %}
Register confidential and public OAuth clients and manage their secrets.
{% /cards_item %}
{% cards_item href="/docs/products/auth/oauth-server/authorization" title="Authorization" %}
The authorization code flow, PKCE, and hosting your own consent screen.
{% /cards_item %}
{% cards_item href="/docs/products/auth/oauth-server/tokens" title="Tokens" %}
Access, refresh, and ID tokens, their lifetimes, introspection, and revocation.
{% /cards_item %}
{% cards_item href="/docs/products/auth/oauth-server/scopes" title="Scopes" %}
The built-in OpenID scopes and the custom scopes your clients can request.
{% /cards_item %}
{% cards_item href="/docs/products/auth/oauth-server/device-flow" title="Device flow" %}
...
{% cards %}
{% cards_item href="/docs/products/auth/oauth-server/sign-in-with-your-product/step-1" title="Sign in with your product" %}
Build a full sign-in with your product experience end to end, from the consent screen to the token exchange.
{% /cards_item %}
{% cards_item href="/docs/products/auth/oauth-server/custom-scopes/step-1" title="Protect your API with custom scopes" %}
Define custom scopes, request them from a client, and enforce them on your own API.
{% /cards_item %}
{% /cards %}
`;export{e as default};