...
# Enable the server {% #enable %}

In the Console, open **Auth**, select the **OAuth2 server** tab, and turn on **Enable OAuth2 server**.

{% only_dark %}
![Enabling the OAuth2 server in the Appwrite Console](/images/docs/oauth-server/dark/oauth2-server-settings.avif)
{% /only_dark %}
{% only_light %}
...
# Copy the discovery URL {% #discovery %}
...
{% only_dark %}
![Creating an OAuth2 client](/images/docs/oauth-server/dark/oauth2-server-create-app.avif)
{% /only_dark %}
{% only_light %}
...
# Run the authorization code flow {% #flow %}
...
## 1. Send the user to the authorization endpoint {% #send-to-authorization %}
...
## 2. Review and approve Vantage's request {% #approve-grant %}
...
## 3. Exchange the code for tokens {% #exchange-code %}
...
## 4. Read the user's profile {% #read-profile %}
...
Host your consent screen and drive the authorize, grant, and approve steps.
{% /cards_item %}
{% cards_item href="/docs/products/auth/oauth-server/clients" title="Clients" %}
Confidential vs public clients, secrets, and rotation.
{% /cards_item %}
{% cards_item href="/docs/products/auth/oauth-server/tokens" title="Tokens" %}
Token lifetimes, refresh with rotation, introspection, and revocation.
{% /cards_item %}
{% /cards %}
`;export{e as default};