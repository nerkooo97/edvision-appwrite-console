...
PostgreSQL exposes a rich extension ecosystem: PostGIS for geospatial data, pgvector for embeddings, pg_trgm for fuzzy search, and more. Native PostgreSQL databases on Appwrite support managing extensions through the API, at no extra cost.

# Install an extension {% #install %}

Pass the extension name as it appears in the [extension catalog](#list). The install runs asynchronously: the request returns immediately and a worker runs \`CREATE EXTENSION\` inside the database, typically within a few seconds. The database must be in the \`ready\` state.
...
The available list is reported by the engine for your PostgreSQL version, so it always reflects what can actually be installed on your database.
