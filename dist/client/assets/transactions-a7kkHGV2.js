...
Call the \`createTransaction\` method to begin. It returns a transaction model that includes \`$id\`. Pass this ID as \`transactionId\` to subsequent operations. Appwrite [Server SDKs](/docs/sdks#server) require an [API key](/docs/advanced/platform/api-keys).
...
\`\`\`
\`\`\`bash
appwrite vectors-db create-transaction
\`\`\`
{% /multicode %}
...
\`\`\`
\`\`\`bash
appwrite vectors-db update-document \\
...
\`\`\`
\`\`\`bash
appwrite vectors-db get-document \\
    --database-id <DATABASE_ID> \\
    --collection-id <COLLECTION_ID> \\
    --document-id <DOCUMENT_ID> \\
    --transaction-id <TRANSACTION_ID>
...
For \`create\`, \`update\`, and \`upsert\` actions, pass the document \`data\` (an \`embeddings\` vector and optional \`metadata\`). For \`delete\`, the \`documentId\` is enough.
...
When you are done staging operations, call the \`updateTransaction\` method to finalize the transaction. Set \`commit\` to \`true\` to apply the staged operations, or \`rollback\` to \`true\` to discard them.
...
\`\`\`bash
# Commit
appwrite vectors-db update-transaction \\
    --transaction-id <TRANSACTION_ID> \\
    --commit true
...
# Inspect and delete transactions {% #inspect-and-delete %}
...
# List transactions
appwrite vectors-db list-transactions

# Delete a transaction
appwrite vectors-db delete-transaction \\
    --transaction-id <TRANSACTION_ID>
\`\`\`
{% /multicode %}
