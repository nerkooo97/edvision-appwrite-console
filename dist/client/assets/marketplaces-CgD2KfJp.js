...
# Step 1: Register apps with the Apps API {% #step-1-register-apps-with-the-apps-api %}
...
const app = await apps.create({
    appId: ID.unique(),
    name: 'Acme Analytics',
    teamId: '<DEVELOPER_OR_PLATFORM_ORG_ID>',
    redirectUris: ['https://marketplace.example.com/oauth/callback'],
    description: 'Usage dashboards for Appwrite projects.',
    tagline: 'Analytics for your backend',
...
// Store secret.secret for token exchange on your server
\`\`\`
{% /multicode %}

Save metadata your marketplace UI needs: \`app.$id\`, name, logo URIs, requested scopes, and developer contact. See [Apps API](/docs/partners/apps) for update, list, and secret rotation.

# Step 2: Build your catalog {% #step-2-build-your-catalog %}
...
# Step 3: Start install with OAuth {% #step-3-start-install-with-oauth %}
...
# Step 4: Exchange tokens and record the install {% #step-4-exchange-tokens-and-record-the-install %}
...
# Step 5: Let integrations act on installed orgs {% #step-5-let-integrations-act-on-installed-orgs %}
...
const userClient = new Client()
    .setEndpoint('https://cloud.appwrite.io/v1')
    .setProject('console')
    .setJWT(await refreshAccessToken(install));

const projects = new Projects(userClient);
const list = await projects.list();
\`\`\`
{% /multicode %}

Alternatively, pass a short-lived token or scoped proxy credential to the integration if you do not want third-party servers to hold long-lived refresh tokens. See [Proxy](/docs/partners/proxy) for wrapping Console APIs behind your platform.
...
# Uninstall and revocation {% #uninstall-and-revocation %}
...
{% cards %}
{% cards_item href="/docs/partners/apps" title="Apps API" %}
Create and manage OAuth app records.
{% /cards_item %}
{% cards_item href="/docs/partners/oauth-connect/setup" title="OAuth connect setup" %}
Authorization code flow and token exchange.
{% /cards_item %}
{% cards_item href="/docs/partners/oauth-connect/scopes" title="OAuth scopes" %}
Request the minimum Console scopes per integration.
{% /cards_item %}
{% /cards %}
`;export{e as default};