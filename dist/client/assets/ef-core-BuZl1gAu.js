...
# Configure the DbContext {% #dbcontext %}
...
Migrations need a session connection and full DDL privileges, so run them against the direct MySQL host on port \`3306\`, not the [pooler](/docs/products/databases/mysql/connection-pooling). The primary \`admin\` user owns the database.
...
# Wire up an ASP.NET server {% #aspnet %}
...
{% cards %}
{% cards_item href="/docs/products/databases/mysql/connections" title="Connect" %}
...
{% /cards_item %}
{% cards_item href="/docs/products/databases/mysql/network-security" title="Network" %}
