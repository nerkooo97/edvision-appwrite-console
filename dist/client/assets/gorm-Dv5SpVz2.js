...
You'll need a native MySQL database in a \`ready\` state and its credentials. See [native MySQL databases](/docs/products/databases/mysql) to create one and [Connections](/docs/products/databases/mysql/connections) to retrieve the hostname, password, and generated database name. The primary user is \`admin\`.
{% /info %}
...
GORM's MySQL driver uses [go-sql-driver/mysql](https://github.com/go-sql-driver/mysql) DSNs. The wire format is \`admin:<password>@tcp(db-<hash>.<region>.appwrite.center:3306)/<database>?parseTime=true&tls=true\`. Use \`mysql.Config\` to format the DSN so passwords and database names are escaped correctly:
...
For mTLS, see the [Network security](/docs/products/databases/mysql/network-security) page.
...
TLS and network controls for native MySQL databases.
{% /cards_item %}
{% /cards %}
`;export{e as default};