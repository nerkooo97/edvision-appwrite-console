...
type DisablePremiumGeoDBDialogProps = {
  open: boolean
...
export function DisablePremiumGeoDBDialog({
  open,
  onOpenChange,
  projectId,
  addonId,
}: DisablePremiumGeoDBDialogProps) {
  const t = useT()
  ...
  const handleDisable = async () => {
    ...
    setError(null)
    try {
      await sdk.forConsole.projects.deleteAddon({ projectId, addonId })
      await Promise.all([
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md p-0">
        <DialogHeader className="px-6 pt-6 pb-4 text-left">
          <DialogTitle>{t('Disable Premium Geo DB')}</DialogTitle>
