...
function getPostgresBackupStatus(status: string) {
...
function isManualPostgresBackup(backup: Models.DedicatedDatabaseBackup) {
...
export function View({ projectId, databaseId }: ViewProps) {
  ...
  const { data: policiesData, isLoading: policiesLoading } =
    usePostgresBackupPolicies(projectId, databaseId)
  const { data: backupsData, isLoading: backupsLoading } = usePostgresBackups(
    projectId,
    databaseId,
    backupsPage - 1,
    backupsPageSize,
  ...
  const invalidateBackups = () => {
    queryClient.invalidateQueries({
      queryKey: ['postgres-backups', 'project', projectId, databaseId],
    })
  }
  ...
  const createPolicyMutation = useMutation({
    mutationFn: async (
      ...
      return Promise.all(
        items.map((policy) =>
          projectSdk.postgresql.createBackupPolicy({
            databaseId,
  ...
  const deletePolicyMutation = useMutation({
    mutationFn: async (policyId: string) => {
      const projectSdk = sdk.forProject(projectId)
      return projectSdk.postgresql.deleteBackupPolicy({
        databaseId,
        policyId,
      })
    },
  ...
  const createBackupMutation = useMutation({
    mutationFn: async () => {
      const projectSdk = sdk.forProject(projectId)
      return projectSdk.postgresql.createBackup({ databaseId })
    },
    onSuccess: () => {
  ...
  const deleteBackupMutation = useMutation({
    mutationFn: async (backupId: string) => {
      const projectSdk = sdk.forProject(projectId)
      return projectSdk.postgresql.deleteBackup({ databaseId, backupId })
    },
    onSuccess: () => {
    ...
    mutationFn: async (backupIds: string[]) => {
      ...
      await Promise.all(
        backupIds.map((backupId) =>
          projectSdk.postgresql.deleteBackup({ databaseId, backupId }),
        ),
      )
    },
    onSuccess: () => {
  ...
  const createRestorationMutation = useMutation({
    mutationFn: async (backupId: string) => {
      const projectSdk = sdk.forProject(projectId)
      return projectSdk.postgresql.createRestoration({
        databaseId,
        type: 'backup',
        backupId,
      })
