...
import {
  ...
  buildPostgresConnectSnippets,
  buildPostgresConnectionEndpointInfo,
  ...
  type PostgresConnectTab,
  type PostgresConnectionEndpointInfo,
  type PostgresDrizzleSnippetPart,
  type PostgresPsqlSnippetPart,
...
const PSQL_SNIPPET_FILES: {
  id: PostgresPsqlSnippetPart
  label: string
}[] = [
...
type PostgresConnectDrizzleProps = {
  context: PostgresConnectSnippetContext
  copyContent: string
}

function PostgresConnectDrizzle({
  context,
  copyContent,
}: PostgresConnectDrizzleProps) {
  const [activeFile, setActiveFile] =
    useState<PostgresDrizzleSnippetPart>('env')

  const activeFileConfig =
    DRIZZLE_SNIPPET_FILES.find((file) => file.id === activeFile) ??
    DRIZZLE_SNIPPET_FILES[0]
  const displayCode = buildPostgresDrizzleSnippetPartDisplayCode(
    activeFile,
    context,
  )
...
type PostgresConnectPsqlProps = {
  context: PostgresConnectSnippetContext
  copyContent: string
}

function PostgresConnectPsql({
  context,
  copyContent,
...
export function PostgresConnectDialog({
  ...
  const endpointInfo = useMemo(
    () =>
      credentials
        ? buildPostgresConnectionEndpointInfo(credentials, pooler)
        : null,
    [credentials, pooler],
  )
  ...
  const snippets = useMemo(
    () =>
      credentials && endpointInfo
        ? buildPostgresConnectSnippets({ credentials, endpointInfo })
        : null,
    [credentials, endpointInfo],
  )
      ...
      <DialogContent className="flex max-h-[min(90dvh,800px)] flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
        ...
        {isLoading ? (
          ...
          <Tabs
            value={methodTab}
            onValueChange={(value) =>
              setMethodTab(value as PostgresConnectTab)
            }
          ...
          >
            ...
            <div className="shrink-0 px-6 py-4">
              ...
              {POSTGRES_CONNECT_SNIPPET_TABS.map((tab) => {
                ...
                const displayCode = buildPostgresConnectSnippetDisplayCode(
                  tab.id,
                  { credentials, endpointInfo },
                )
                const panelLabel =
