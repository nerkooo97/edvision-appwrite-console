...
import {
  ...
  POSTGRES_ROWS_DATA_COLUMN_MIN_WIDTH_PX,
...
function clampColumnWidth(width: number): number {
  return Math.min(
    POSTGRES_ROWS_DATA_COLUMN_MAX_WIDTH_PX,
    Math.max(POSTGRES_ROWS_DATA_COLUMN_MIN_WIDTH_PX, width),
  )
}

export function usePostgresRowsColumnResize(
  databaseId: string,
  tableId: string,
  columnKeys: string[],
  ...
  const getColumnWidthPx = useCallback(
    (columnKey: string) => {
      ...
      return POSTGRES_ROWS_DATA_COLUMN_DEFAULT_WIDTH_PX
    },
    [columnWidths],
  )
