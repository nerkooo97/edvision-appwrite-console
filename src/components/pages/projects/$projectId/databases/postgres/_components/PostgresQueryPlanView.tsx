...
import {
  countPostgresQueryPlanNodes,
  formatPostgresQueryPlanRaw,
  normalizePostgresQueryPlan,
  type PostgresQueryPlanNode,
...
const VARIANT_GUIDE: Record<PostgresQueryPlanNodeVariant, string> = {
...
export function PostgresQueryPlanView({
  ...
  const planNodes = useMemo(
    () => (explanation ? normalizePostgresQueryPlan(explanation) : []),
    [explanation],
  )
  const nodeCount = countPostgresQueryPlanNodes(planNodes)
  const rawPlan = useMemo(
    () => (explanation ? formatPostgresQueryPlanRaw(explanation) : ''),
    [explanation],
  )
  const rawLanguage =
