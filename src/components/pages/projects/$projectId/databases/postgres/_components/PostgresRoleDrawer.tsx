...
type PostgresRoleDrawerProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  projectId: string
  databaseId: string
  role?: PostgresRoleRow | null
  availableRoles: PostgresRoleRow[]
  onSuccess: () => void
}
...
export function PostgresRoleDrawer({
  open,
  onOpenChange,
  projectId,
  databaseId,
  ...
  availableRoles,
  onSuccess,
}: PostgresRoleDrawerProps) {
  const t = useT()
  const executeSql = useExecutePostgresSql(projectId, databaseId)
  const isEdit = Boolean(role)

  const [formState, setFormState] = useState<PostgresRoleFormState>(
    createDefaultPostgresRoleFormState(),
  )
  ...
  const updateForm = (patch: Partial<PostgresRoleFormState>) => {
  ...
  return (
    <BaseDrawer
      ...
      description={
        isEdit
          ? t('Update PostgreSQL role attributes, limits, and membership.')
          : t('Create a PostgreSQL role for RLS policies and database access.')
      }
