...
import {
  usePostgresSchemaVisualizer,
  type PostgresVisualizerColumn,
  type PostgresVisualizerRelation,
...
function isViewRelation(relation: Pick<PostgresVisualizerRelation, 'tableType'>) {
  return relation.tableType.toUpperCase() === 'VIEW'
}

function formatRelationLabel(
  relation: Pick<PostgresVisualizerRelation, 'schema' | 'name' | 'isExternal'>,
) {
...
export function PostgresSchemaVisualizer({ databaseId }: SchemaVisualizerProps) {
  ...
  const {
    ...
    isComplete,
    error,
  } = usePostgresSchemaVisualizer(projectId, databaseId, selectedSchema)
  ...
  const handleNavigateToRows = (relationId: string) => {
    navigate(postgresNav({ projectId, databaseId }).table({ tableId: relationId }).rows())
  }

  const handleNavigateToColumns = (relationId: string) => {
    navigate(
      postgresNav({ projectId, databaseId }).table({ tableId: relationId }).columns(),
    )
  }
              ...
              {node.columns.length > 0 ? (
                <div className="space-y-0.5">
                  {visibleColumns.map((column) => {
                    ...
                    const typeLabel = formatPostgresColumnType({
