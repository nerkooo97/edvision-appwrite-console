...
import { useMysqlSqlEditorHeight } from '@/lib/react-query/hooks'
import {
  clampMysqlSqlEditorHeightPx,
  MYSQL_SQL_EDITOR_DEFAULT_HEIGHT_PX,
  MYSQL_SQL_EDITOR_MAX_HEIGHT_PX,
  MYSQL_SQL_EDITOR_MIN_HEIGHT_PX,
  MYSQL_SQL_RESULTS_MIN_HEIGHT_PX,
...
type MysqlSqlEditorContainerProps = {
  editor: ReactNode
  children: ReactNode
  className?: string
}
...
function clampEditorHeightInContainer(
  ...
  const maxInContainer = Math.min(
    ...
    Math.max(
      MYSQL_SQL_EDITOR_MIN_HEIGHT_PX,
      containerHeightPx - MYSQL_SQL_RESULTS_MIN_HEIGHT_PX,
    ),
  )
  return Math.min(maxInContainer, Math.max(MYSQL_SQL_EDITOR_MIN_HEIGHT_PX, clamped))
}
 ...
 */
export function MysqlSqlEditorContainer({
  editor,
  children,
  className,
}: MysqlSqlEditorContainerProps) {
  const t = useT()
  ...
  const { heightPx: persistedHeightPx, persistEditorHeightPx } =
    useMysqlSqlEditorHeight(accountPrefs)

  const initialHeightPx = clampMysqlSqlEditorHeightPx(
    persistedHeightPx ?? MYSQL_SQL_EDITOR_DEFAULT_HEIGHT_PX,
  )
