...
import { MYSQL_BODY_CELL_BORDER_CLASS } from './mysql-spreadsheet-chrome'
...
export function MysqlInlineTableCell({
  tableId,
  rowKey,
  column,
  identity,
  ...
  const editable =
    ...
    isMysqlColumnInlineEditable(column)
  const fieldType = getMysqlInlineFieldType(meta, originalValue)
  const isRequired = isMysqlColumnRequired(column)
  const displayValue =
  ...
  const startEditing = useCallback(
    (event?: MouseEvent) => {
      ...
      if (!started) return
      setDraft(valueToMysqlEditString(displayValue, meta))
      setIsEditing(true)
    },
    [
      column.column_name,
