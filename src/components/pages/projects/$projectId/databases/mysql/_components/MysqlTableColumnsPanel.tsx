...
import {
  useExecuteMysqlSql,
  useMysqlTableColumns} from '@/lib/react-query/hooks'
import {
  buildMysqlDropColumnSql,
  formatMysqlColumnType} from '@/lib/mysql-table-ddl'
import {
  formatMysqlColumnMetadataPreview,
  getMysqlColumnCheckDisplay,
  getMysqlColumnForeignKeyDisplay} from '@/lib/mysql-column-metadata'
import { getErrorMessage } from '@/lib/utils/error-formatting'
import {
  isMysqlPrimaryKeyColumn,
  type MysqlTableColumnRow} from '@/lib/mysql-sql'
import { CheckCircle2, Copy, Pencil, Trash2 } from 'lucide-react'
import { MysqlTableColumnDrawer } from './MysqlTableColumnDrawer'
import { MysqlColumnContextMenu } from './MysqlColumnContextMenu'
...
import {
  ...
  MYSQL_STICKY_ACTIONS_HEADER_CLASS,
...
export function MysqlTableColumnsPanel({
  databaseId,
  tableId,
  search = '',
  createDialogOpen: createDialogOpenProp,
  onCreateDialogOpenChange}: MysqlTableColumnsPanelProps) {
  const t = useT()
  const { projectId } = useParams({ strict: false }) as { projectId: string }
  const { canWrite } = useDatabaseTableOperationsAccess()

  const { columns, isLoading, refetch } = useMysqlTableColumns(
    projectId,
    databaseId,
    tableId,
  )
  ...
  const [selectedColumn, setSelectedColumn] =
    useState<MysqlTableColumnRow | null>(null)
  ...
  const handleEdit = (column: MysqlTableColumnRow) => {
  ...
  const filteredColumns = useMemo(() => {
    return columns.filter((column) => {
      ...
      return matchesMysqlLocalSearch(
        ...
        getMysqlColumnCheckDisplay(column.check_constraints),
        getMysqlColumnForeignKeyDisplay(column.foreign_keys),
      )
    })
  }, [columns, search])
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {filteredColumns.map((column) => {
                ...
                const checkDisplay = getMysqlColumnCheckDisplay(
                  column.check_constraints,
                )
                const foreignKeyDisplay = getMysqlColumnForeignKeyDisplay(
                  column.foreign_keys,
                )
                const commentDisplay = column.column_comment?.trim() ?? ''

                return (
                  <MysqlColumnContextMenu
                  ...
                  >
                    <td className={cn('px-3 py-2', MYSQL_BODY_CELL_BORDER_CLASS)}>
                    ...
                    <td className={cn('px-3 py-2', MYSQL_BODY_CELL_BORDER_CLASS)}>
                      <Badge
                        ...
                        className={cn(
                          ...
                          getMysqlColumnTypeColor(typeKey),
                        )}
                      >
                        {typeLabel}
                      </Badge>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <code className="block max-w-[220px] truncate font-mono text-[11px] text-foreground">
                              {formatMysqlColumnMetadataPreview(checkDisplay, 40)}
                            </code>
                          </TooltipTrigger>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span className="block max-w-[220px] truncate text-[12px] text-foreground">
                              {formatMysqlColumnMetadataPreview(commentDisplay, 40)}
                            </span>
                          </TooltipTrigger>
                          <TooltipContent side="top" className="max-w-sm">
