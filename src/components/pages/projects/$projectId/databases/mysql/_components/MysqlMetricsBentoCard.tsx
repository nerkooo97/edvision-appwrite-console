...
export type MysqlMetricsBentoTile = {
  id: string
  label: string
  value: ReactNode
  subValue?: string
...
type MysqlMetricsBentoCardProps = {
  title?: string
  tiles: MysqlMetricsBentoTile[]
...
export function MysqlMetricsBentoCard({
  title,
  tiles,
  columns = 3,
  className,
}: MysqlMetricsBentoCardProps) {
    ...
    >
      ...
      <div className="divide-y divide-border">
        {rows.map((row, rowIndex) => (
          ...
          >
            {row.map((tile) => (
              <MysqlMetricBentoTile key={tile.id} {...tile} />
            ))}
          </div>
        ))}
      </div>
