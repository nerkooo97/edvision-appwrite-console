...
import {
  formatMysqlQueryDurationMs,
} from '@/lib/mysql-sql'
import { useT } from '@/lib/i18n/translate'

type MysqlQueryResultsMetaProps = {
  title: string
  rowCount: number
  countLabel?: string
  durationMs?: number
...
export function MysqlQueryResultsMeta({
  title,
  rowCount,
  countLabel = 'row',
  durationMs,
  truncated,
}: MysqlQueryResultsMetaProps) {
  const t = useT()
          ...
          {durationMs != null ? (
            <>
              ...
              <span className="tabular-nums">
                {formatMysqlQueryDurationMs(durationMs)}
              </span>
            </>
          ) : null}
        </p>
