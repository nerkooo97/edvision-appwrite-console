...
import {
  useExecuteMysqlSql,
  useMysqlTableIndexes} from '@/lib/react-query/hooks'
import { buildMysqlDropIndexSql } from '@/lib/mysql-table-ddl'
import { parseMysqlTableId } from '@/lib/mysql-database-routes'
import {
  formatMysqlIndexMetadataPreview,
  parseMysqlIndexIncludeColumns} from '@/lib/mysql-index-metadata'
import { getErrorMessage } from '@/lib/utils/error-formatting'
import {
  isMysqlPrimaryIndex,
  type MysqlTableIndexRow} from '@/lib/mysql-sql'
import { Key, Trash2 } from 'lucide-react'
import { MysqlTableIndexDrawer } from './MysqlTableIndexDrawer'
import { MysqlIndexContextMenu } from './MysqlIndexContextMenu'
...
export function MysqlTableIndexesPanel({
  ...
  const filteredIndexes = useMemo(() => {
    return indexes.filter((index) => {
      ...
      const indexColumns = parseMysqlIndexColumnsFromDefinition(
        index.index_definition,
      ).join(' ')
      const includeColumns =
        index.index_include ??
        parseMysqlIndexIncludeColumns(index.index_definition).join(', ')
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {filteredIndexes.map((index) => {
                ...
                const isUnique =
                  isTruthyFlag(index.is_unique) || isMysqlPrimaryIndex(index)

                return (
                  <MysqlIndexContextMenu
                  ...
                  >
                    ...
                    <td className={cn('px-3 py-2', MYSQL_BODY_CELL_BORDER_CLASS)}>
                      <Badge variant="outline" className="text-[11px] font-medium">
                        {localizeMysqlIndexAlgorithmLabel(
                          index.index_algorithm,
                          t,
                        )}
                      </Badge>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <code className="block max-w-[220px] truncate font-mono text-[11px] text-foreground">
                              {formatMysqlIndexMetadataPreview(condition, 40)}
                            </code>
                          </TooltipTrigger>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <code className="block max-w-[180px] truncate font-mono text-[11px] text-foreground">
                              {formatMysqlIndexMetadataPreview(includeColumns, 32)}
                            </code>
                          </TooltipTrigger>
                          <TooltipContent side="top" className="max-w-sm">
                            <p className="font-mono text-[11px] break-all">
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span className="block max-w-[220px] truncate text-[12px] text-foreground">
                              {formatMysqlIndexMetadataPreview(comment, 40)}
                            </span>
                          </TooltipTrigger>
                          <TooltipContent side="top" className="max-w-sm">
