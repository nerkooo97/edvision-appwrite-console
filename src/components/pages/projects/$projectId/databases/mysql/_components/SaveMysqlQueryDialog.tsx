...
import { queryPreviewLabel, useMysqlSidebar } from './MysqlSidebarContext'
import { MAX_SAVED_MYSQL_QUERY_NAME_LENGTH } from '@/lib/user-prefs-keys'
import { useT } from '@/lib/i18n/translate'

type SaveMysqlQueryDialogProps = {
  open: boolean
...
export function SaveMysqlQueryDialog({
  open,
  onOpenChange,
  databaseId,
  sql,
  ...
  teamId,
  canSaveTeam,
}: SaveMysqlQueryDialogProps) {
  const t = useT()
  const [name, setName] = useState('')
  const [level, setLevel] = useState<MysqlSavedQueryLevel>('user')
  const { setPanel, setSavedQueryLevel, selectSavedQuery, savedQueryLevel } =
    useMysqlSidebar()
  const { addSavedQuery, isAdding, hasTeamLevel } = useMysqlSavedQueries(
    databaseId,
    account,
    teamId,
  )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md p-0">
        ...
        <div className="space-y-4 px-6 py-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="mysql-query-name" className="text-[13px]">
              {t('Query name')}
            </Label>
            <Input
              id="mysql-query-name"
              ...
              maxLength={MAX_SAVED_MYSQL_QUERY_NAME_LENGTH}
              disabled={isAdding}
            />
          </div>
        </div>
