...
type MysqlRowsEditSessionContextValue = {
  ...
  setCellEdit: (edit: Omit<PendingMysqlRowCellEdit, 'databaseId'>) => void
  discardAll: () => void
...
const MysqlRowsEditSessionContext =
  createContext<MysqlRowsEditSessionContextValue | null>(null)

export function useMysqlRowsEditSession() {
  return useContext(MysqlRowsEditSessionContext)
}

type MysqlRowsEditSessionProviderProps = {
  projectId: string
  databaseId: string
  tableId: string
  canWrite?: boolean
...
export function MysqlRowsEditSessionProvider({
  projectId,
  databaseId,
  tableId,
  canWrite = true,
  children,
}: MysqlRowsEditSessionProviderProps) {
  const t = useT()
  const [pendingEdits, setPendingEdits] = useState<
    Map<string, PendingMysqlRowCellEdit>
  >(() => new Map())
  ...
  const commitMutation = useCommitMysqlRowEdits(
    projectId,
    databaseId,
    tableId,
  )
  ...
  const beginInlineEdit = useCallback(
    (params: {
      ...
      const key = makeMysqlPendingEditKey(
        params.tableId,
        params.rowKey,
        params.columnKey,
      )
  ...
  const endInlineEdit = useCallback(
    (editTableId: string, rowKey: string, columnKey: string) => {
      const key = makeMysqlPendingEditKey(editTableId, rowKey, columnKey)
  ...
  const setCellEdit = useCallback(
    (edit: Omit<PendingMysqlRowCellEdit, 'databaseId'>) => {
      if (!canWrite) return
      const key = makeMysqlPendingEditKey(
        edit.tableId,
        edit.rowKey,
  ...
  return (
    <MysqlRowsEditSessionContext.Provider value={value}>
      {children}
      {pendingEdits.size > 0 ? (
    ...
    </MysqlRowsEditSessionContext.Provider>
  )
}
