...
import {
  ...
  buildMysqlEnableRlsSql,
  buildMysqlForceRlsSql,
  buildMysqlNoForceRlsSql,
  formatMysqlPolicyRoles,
  normalizeMysqlPolicyCommand,
  type MysqlTablePolicyRow,
} from '@/lib/mysql-rls'
import { getErrorMessage } from '@/lib/utils/error-formatting'
import { Pencil, Plus, Search, Shield, Trash2 } from 'lucide-react'
import { MysqlTablePolicyDrawer } from './MysqlTablePolicyDrawer'
import { useT } from '@/lib/i18n/translate'
import {
  matchesMysqlLocalSearch,
} from './mysql-spreadsheet-chrome'

type MysqlTableSecurityPanelProps = {
  databaseId: string
  tableId: string
}
...
export function MysqlTableSecurityPanel({
  databaseId,
  tableId,
}: MysqlTableSecurityPanelProps) {
  const t = useT()
  ...
  const {
    ...
    refetch: refetchRls,
  } = useMysqlTableRls(projectId, databaseId, tableId)
  const {
    policies,
    isLoading: policiesLoading,
    isFetching: policiesFetching,
    refetch: refetchPolicies,
  } = useMysqlTablePolicies(projectId, databaseId, tableId)
  const executeSql = useExecuteMysqlSql(projectId, databaseId)

  const [searchValue, setSearchValue] = useState('')
  const [dialogOpen, setDialogOpen] = useState(false)
  const [selectedPolicy, setSelectedPolicy] =
    useState<MysqlTablePolicyRow | null>(null)
  ...
  const handleOpenEdit = (policy: MysqlTablePolicyRow) => {
      ...
      if (effectiveRowSecurity !== rowSecurityEnabled) {
        statements.push(
          effectiveRowSecurity
            ? buildMysqlEnableRlsSql(tableId)
            : buildMysqlDisableRlsSql(tableId),
        )
      }
      if (effectiveForceRowSecurity !== forceRowSecurity) {
        statements.push(
          effectiveForceRowSecurity
            ? buildMysqlForceRlsSql(tableId)
            : buildMysqlNoForceRlsSql(tableId),
        )
      }
  ...
  const handleDeletePolicy = async () => {
    ...
    try {
      await executeSql.mutateAsync(
        buildMysqlDropPolicySql(tableId, policyToDelete),
      )
  ...
  const filteredPolicies = useMemo(() => {
    return policies.filter((policy) =>
      matchesMysqlLocalSearch(
        ...
        policy.cmd,
        policy.permissive,
        formatMysqlPolicyRoles(policy.roles),
        policy.qual ?? '',
        policy.with_check ?? '',
      ),
    )
                    ...
                    <TableRow key={policy.policyname}>
                      ...
                      <TableCell className="px-4 py-3">
                        <Badge variant="info" className="text-[10px] shrink-0">
                          {normalizeMysqlPolicyCommand(policy.cmd)}
                        </Badge>
                      </TableCell>
                      <TableCell className="px-4 py-3">
