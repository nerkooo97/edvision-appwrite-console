...
import {
  addMysqlPolicyFormRole,
  buildMysqlAlterPolicySql,
  buildMysqlCreatePolicySql,
  createDefaultMysqlPolicyFormState,
  formatMysqlPolicyFormRoles,
  mapMysqlPolicyRowToFormState,
  normalizeMysqlTablePolicyRow,
  parseMysqlPolicyFormRoles,
  removeMysqlPolicyFormRole,
  validateMysqlPolicyFormState,
  type MysqlPolicyCommand,
  type MysqlPolicyFormState,
  type MysqlPolicyPermissive,
  type MysqlTablePolicyRow,
} from '@/lib/mysql-rls'
import { isMysqlBuiltinRole } from '@/lib/mysql-roles'
import { useExecuteMysqlSql, useMysqlRoles } from '@/lib/react-query/hooks'
import { getErrorMessage } from '@/lib/utils/error-formatting'
import { useT } from '@/lib/i18n/translate'

type MysqlTablePolicyDrawerProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  projectId: string
  databaseId: string
  tableId: string
  policy?: MysqlTablePolicyRow | null
  onSuccess: () => void
}

const POLICY_COMMANDS: MysqlPolicyCommand[] = [
  'ALL',
  'SELECT',
  'INSERT',
  'UPDATE',
...
type MysqlPolicyRolesFieldProps = {
  projectId: string
  databaseId: string
  value: string
...
function MysqlPolicyRolesField({
  projectId,
  databaseId,
  value,
  onChange,
}: MysqlPolicyRolesFieldProps) {
  const t = useT()
    ...
    const databaseOptions = roles
      ...
      .map((roleName) => {
        ...
        return {
          value: roleName,
          label: roleName,
          description: row && isMysqlBuiltinRole(row) ? t('System') : undefined,
        }
      })
  ...
  const removeRole = (roleName: string) => {
    onChange(
      formatMysqlPolicyFormRoles(
        removeMysqlPolicyFormRole(selectedRoles, roleName),
      ),
    )
  }
...
export function MysqlTablePolicyDrawer({
  open,
  onOpenChange,
  projectId,
  databaseId,
  ...
  policy,
  onSuccess,
}: MysqlTablePolicyDrawerProps) {
  const t = useT()
  const executeSql = useExecuteMysqlSql(projectId, databaseId)
  const isEdit = Boolean(policy)

  const policySnapshot = useMemo(
    () => (policy ? normalizeMysqlTablePolicyRow(policy) : null),
    [policy],
  )

  const [formState, setFormState] = useState<MysqlPolicyFormState>(() =>
    policy
      ? mapMysqlPolicyRowToFormState(policy)
      : createDefaultMysqlPolicyFormState(),
  )
  ...
  useEffect(() => {
    ...
    setFormState(
      policySnapshot
        ? mapMysqlPolicyRowToFormState(policySnapshot)
        : createDefaultMysqlPolicyFormState(),
    )
  }, [open, policySnapshot])
  ...
  const handleSubmit = async () => {
    const validationError = validateMysqlPolicyFormState(formState, {
      isEdit,
    })
    if (validationError) {
    ...
    try {
      const sql = isEdit
        ? buildMysqlAlterPolicySql(tableId, policy!.policyname, formState)
        : buildMysqlCreatePolicySql(tableId, formState)
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setFormState((prev) => ({
                      ...prev,
                      command: value as MysqlPolicyCommand,
                    }))
                  }
                  disabled={isEdit}
                >
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setFormState((prev) => ({
                      ...prev,
                      permissive: value as MysqlPolicyPermissive,
                    }))
                  }
                  disabled={isEdit}
                >
