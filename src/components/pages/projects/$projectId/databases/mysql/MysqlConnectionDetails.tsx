...
import {
  useCancelMysqlBackend,
  useMysqlActiveConnections,
  useTerminateMysqlBackend,
  useTerminateMysqlIdleInTransaction,
...
import {
  ...
  matchesMysqlConnectionBackendScope,
  matchesMysqlConnectionStateFilter,
  type MysqlActiveConnectionRow,
  type MysqlConnectionBackendScope,
  type MysqlConnectionStateFilter,
...
import { MysqlConnectionDrawer } from './_components/MysqlConnectionDrawer'
import { MysqlConnectionContextMenu } from './_components/MysqlConnectionContextMenu'
import { MysqlConnectionRowActionsMenu } from './_components/MysqlConnectionRowActionsMenu'
...
type MysqlConnectionDetailsProps = {
  projectId: string
  databaseId: string
...
type PendingConnectionAction =
  | { type: 'cancel'; connection: MysqlActiveConnectionRow }
  | { type: 'terminate'; connection: MysqlActiveConnectionRow }
  | { type: 'terminate-idle' }

const BACKEND_SCOPES: Array<{
  id: MysqlConnectionBackendScope
  label: string
}> = [
...
export function MysqlConnectionDetails({
  projectId,
  databaseId,
  centerInPanel = false,
}: MysqlConnectionDetailsProps) {
  const t = useT()
  ...
  const [backendScope, setBackendScope] =
    useState<MysqlConnectionBackendScope>('clients')
  const [stateFilter, setStateFilter] =
    useState<MysqlConnectionStateFilter>('all')
  const [selectedConnection, setSelectedConnection] =
    useState<MysqlActiveConnectionRow | null>(null)
  ...
  const {
    ...
    error,
    refetch,
  } = useMysqlActiveConnections(projectId, databaseId)
  ...
  const terminateIdleMutation = useTerminateMysqlIdleInTransaction(
    projectId,
    databaseId,
  )
  ...
  const scopedConnections = useMemo(
    () =>
      connections.filter((connection) =>
        matchesMysqlConnectionBackendScope(connection, backendScope),
      ),
  ...
  const filteredConnections = useMemo(
    () =>
      scopedConnections.filter((connection) =>
        matchesMysqlConnectionStateFilter(connection, stateFilter),
      ),
  ...
  const backendConnectionCount = useMemo(
    () => connections.filter((connection) => !isMysqlClientBackend(connection)).length,
    [connections],
  )
  ...
  const idleInTransactionCount = useMemo(
    () =>
      connections.filter(
        (connection) =>
          isMysqlClientBackend(connection) &&
          matchesMysqlConnectionStateFilter(
            connection,
            'idle in transaction',
          ),
  ...
  const filterCounts = useMemo(() => {
    const counts: Record<MysqlConnectionStateFilter, number> = {
    ...
    for (const connection of scopedConnections) {
      ...
      if (
        matchesMysqlConnectionStateFilter(connection, 'idle in transaction')
      ) {
  ...
  const openDrawer = useCallback((connection: MysqlActiveConnectionRow) => {
          ...
          {isLoading && connections.length === 0 ? (
                ...
                <Table withScrollContainer={false} className={connectionsTableClassName}>
                  ...
                  <TableBody>
                    {filteredConnections.map((connection) => {
                      ...
                      const databaseLabel = formatMysqlConnectionDatabase(
                        connection.database,
                      )
                      ...
                      const typeLabel = localizeMysqlBackendTypeLabel(
                        connection.backendType,
                        t,
                      )

                      return (
                        <MysqlConnectionContextMenu
                          key={connection.pid}
            ...
            <EmptyState
              ...
              description={
                ...
                clientConnectionCount === 0 &&
                backendConnectionCount > 0
                  ? t('Only MySQL system backends are running. Switch to Backends to inspect them.')
                  : backendScope === 'backends' &&
                      backendConnectionCount === 0 &&
                      clientConnectionCount > 0
