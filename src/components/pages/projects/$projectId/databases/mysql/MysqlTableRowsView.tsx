...
import { MysqlRowsSpreadsheet } from './_components/MysqlRowsSpreadsheet'
import {
  MysqlRowsEditSessionProvider,
  useMysqlRowsEditSession,
} from './_components/MysqlRowsEditSession'
import { MysqlRowEditDrawer } from './_components/MysqlRowEditDrawer'
...
export type MysqlTableRowsViewProps = {
  databaseId: string
  tableId: string
}

export function MysqlTableRowsView({
  databaseId,
  tableId,
}: MysqlTableRowsViewProps) {
  const t = useT()
  ...
  const handleToggleRow = useCallback(
    (rowKey: string, identity: MysqlRowIdentity) => {
  ...
  const handleToggleAllRows = useCallback(
    (
      pageRows: Array<{ rowKey: string; identity: MysqlRowIdentity }>,
      selectAll: boolean,
    ) => {
      setSelectedRows((prev) => {
        const next = new Map(prev)
  ...
  const openRowInDrawer = useCallback(
    (row: Record<string, unknown>, focusedField?: string) => {
      const identity = buildMysqlRowIdentityFromRow(row, tableColumns)
      const rowForDrawer = { ...row }
      delete rowForDrawer[MYSQL_ROW_CTID_COLUMN]
  ...
  const drawerRowKey = useMemo(() => {
    ...
    return getMysqlRowKey(drawerRow, tableColumns)
  }, [drawerIdentity, drawerRow, tableColumns])

  const filterColumns = useMemo(
    () => mysqlRowsFilterColumns(tableColumns),
    [tableColumns],
  )
  const filterScope = `mysql.rows.${databaseId}.${tableId}`
...
type MysqlRowsBulkDeleteProps = {
  projectId: string
  databaseId: string
  tableId: string
  selectedRows: Map<string, MysqlRowIdentity>
  dialogOpen: boolean
...
function MysqlRowsBulkDelete({
  ...
  onClearSelection,
  onDeleted,
}: MysqlRowsBulkDeleteProps) {
  const t = useT()
  const editSession = useMysqlRowsEditSession()
  const deleteMutation = useDeleteMysqlTableRows(
    projectId,
    databaseId,
    tableId,
  )
