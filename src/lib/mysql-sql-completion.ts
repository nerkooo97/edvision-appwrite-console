...
import type { MysqlColumnRow, MysqlTableRow } from '@/lib/mysql-sql'

export type MysqlSqlCompletionTableRef = {
  schema: string
  table: string
}

export type MysqlSqlCompletionResolvers = {
  searchSchemas: (search: string) => Promise<string[]>
  searchTables: (
    schema: string | undefined,
    search: string,
  ) => Promise<MysqlTableRow[]>
  resolveTableColumns: (
    tables: MysqlSqlCompletionTableRef[],
  ) => Promise<MysqlColumnRow[]>
  resolveTableRef: (ref: string) => Promise<MysqlSqlCompletionTableRef | null>
}
...
async function resolveReferencedTables(
  text: string,
  resolveTableRef: MysqlSqlCompletionResolvers['resolveTableRef'],
): Promise<MysqlSqlCompletionTableRef[]> {
  const tokens = extractReferencedTableTokens(text)
  const resolved: MysqlSqlCompletionTableRef[] = []
  const seen = new Set<string>()
...
async function buildSuggestions(
  monaco: Monaco,
  resolvers: MysqlSqlCompletionResolvers,
  textUntilPosition: string,
  prefix: string,
  range: IRange,
...
export function registerMysqlSqlCompletionProvider(
  monaco: Monaco,
  getResolvers: () => MysqlSqlCompletionResolvers,
): IDisposable {
