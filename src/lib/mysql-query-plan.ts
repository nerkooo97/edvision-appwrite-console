...
export type MysqlQueryPlanMetric = {
  label: string
  value: string
}

export type MysqlQueryPlanNode = {
  id: string
  label: string
  nodeType: string
  details: string[]
  metrics: MysqlQueryPlanMetric[]
  children: MysqlQueryPlanNode[]
  variant: MysqlQueryPlanNodeVariant
}
...
function classifyNodeType(nodeType: string): MysqlQueryPlanNodeVariant {
...
function parseCostMetrics(text: string): MysqlQueryPlanMetric[] {
  const metrics: MysqlQueryPlanMetric[] = []
...
function createTextPlanNode(text: string): MysqlQueryPlanNode {
...
function parseExplainTextTree(raw: string): MysqlQueryPlanNode[] {
  const roots: MysqlQueryPlanNode[] = []
  const stack: { indent: number; node: MysqlQueryPlanNode }[] = []
...
function parseJsonPlanNode(raw: Record<string, unknown>): MysqlQueryPlanNode {
...
export function normalizeMysqlQueryPlan(
  explanation: DedicatedDatabaseQueryExplanation,
): MysqlQueryPlanNode[] {
  planNodeId = 0
...
export function formatMysqlQueryPlanRaw(
  explanation: DedicatedDatabaseQueryExplanation,
): string {
  return formatRawPlanForDisplay(explanation)
}

export function countMysqlQueryPlanNodes(
  nodes: MysqlQueryPlanNode[],
): number {
  return nodes.reduce(
    (total, node) => total + 1 + countMysqlQueryPlanNodes(node.children),
    0,
  )
}
