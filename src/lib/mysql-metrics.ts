...
import { executionResultRows } from '@/lib/mysql-sql'

export type MysqlMetricsSnapshotRow = {
...
export type MysqlMetricsSnapshot = {
  timestamp: number
...
export type MysqlConnectionStateRow = {
  state: string
  count: number
}

export type MysqlConnectionAppRow = {
  applicationName: string
  count: number
}

export type MysqlActiveConnectionRow = {
  pid: number
...
export type MysqlTableActivityRow = {
  schema: string
  tableName: string
  totalBytes: number
  liveTuples: number
...
export const MYSQL_LONG_RUNNING_QUERY_THRESHOLD_MS = 10_000
...
/** Human-readable MySQL server uptime from the Uptime status variable. */
export function formatMysqlUptime(seconds: number): string {
...
export function parseMysqlMetricsSnapshot(
  execution: Models.DedicatedDatabaseExecution,
  timestamp = Date.now(),
): MysqlMetricsSnapshot | null {
  const rows = executionResultRows<MysqlMetricsSnapshotRow>(execution)
  const row = rows[0]
  if (!row) return null
...
export function parseMysqlConnectionStates(
  execution: Models.DedicatedDatabaseExecution,
): MysqlConnectionStateRow[] {
...
export function parseMysqlConnectionApps(
  execution: Models.DedicatedDatabaseExecution,
): MysqlConnectionAppRow[] {
...
function readMysqlMetricsRowString(
...
function readMysqlMetricsRowBoolean(
  row: Record<string, unknown>,
  ...keys: string[]
): boolean | null {
  for (const key of keys) {
...
function normalizeMysqlBackendType(
...
function inferMysqlClientBackend(row: {
  ...
  if (normalizedType === MYSQL_CLIENT_BACKEND_TYPE) return true
...
export function parseMysqlActiveConnections(
  execution: Models.DedicatedDatabaseExecution,
): MysqlActiveConnectionRow[] {
  return executionResultRows<Record<string, unknown>>(execution).map((row) => {
    ...
    const parsedIsClientBackend = readMysqlMetricsRowBoolean(
      row,
      'is_client_backend',
      'isClientBackend',
    )
    const isClientBackend =
      ...
      inferMysqlClientBackend({ backendType, state, username, database })
    ...
    return {
      ...
      username,
      database,
      applicationName: readMysqlMetricsRowString(
        row,
        'application_name',
        'applicationName',
      ),
      ...
      state,
      waitEventType: readMysqlMetricsRowString(
        row,
        'wait_event_type',
        'waitEventType',
      ),
...
export const MYSQL_CLIENT_BACKEND_TYPE = 'client backend'

export type MysqlConnectionBackendScope = 'clients' | 'backends'

export function isMysqlClientBackend(
  connection: Pick<MysqlActiveConnectionRow, 'isClientBackend'> &
    Partial<
      Pick<
        MysqlActiveConnectionRow,
...
export function backendTypeBadgeVariant(
  ...
  if (normalizeMysqlBackendType(backendType) === MYSQL_CLIENT_BACKEND_TYPE) {
    return 'success'
  }
  if (normalizeMysqlBackendType(backendType) === 'autovacuum worker') {
    return 'warning'
  }
  return 'info'
}

export function formatMysqlConnectionUsername(
  username: string | null,
  backendType: string | null,
): string {
...
export function formatMysqlConnectionDatabase(database: string | null): string {
  return database?.trim() || '-'
}

export function formatMysqlApplicationName(
  applicationName: string | null,
): string {
  return applicationName?.trim() || '-'
}

export function formatMysqlConnectionStateLabel(
  state: string | null,
  backendType: string | null,
): string {
...
export function formatMysqlClientAddress(
  clientHost: string,
  clientPort: number | null,
): string {
...
export function connectionStateBadgeVariant(
  ...
  if (!state?.trim()) {
    return isMysqlClientBackend({ backendType }) ? 'info' : 'info'
  }
...
export function parseMysqlTableActivity(
  execution: Models.DedicatedDatabaseExecution,
): MysqlTableActivityRow[] {
  return executionResultRows<{
    schemaname?: string
    table_name?: string
...
export type MysqlConnectionStateFilter =
  | 'all'
  | 'active'
  | 'idle'
  | 'idle in transaction'
...
export function matchesMysqlConnectionStateFilter(
  connection: MysqlActiveConnectionRow,
  filter: MysqlConnectionStateFilter,
): boolean {
...
export function isLongRunningConnection(
  ...
  >,
  thresholdMs = MYSQL_LONG_RUNNING_QUERY_THRESHOLD_MS,
): boolean {
...
export function formatMysqlDurationSince(isoDate: string | null): string {
  if (!isoDate) return '-'
...
export function formatMysqlWaitEvent(
  waitEventType: string | null,
  waitEvent: string | null,
): string {
...
export function serializeMysqlActiveConnectionJson(
  connection: MysqlActiveConnectionRow,
): string {
  return JSON.stringify(
    {
      ...
      clientAddress: formatMysqlClientAddress(
